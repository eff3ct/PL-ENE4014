...
Expr eval_under_env(Expr e, std::map<string, Expr> env) {
    return std::visit(overload {
        ...
        [&](box<struct IsAUnit>& isa) { 
            ...
            if (is<AUnit>(e1)) return Expr(Int(1));
        ...
        [&](box<struct MLet>& l) {
            ...
            newEnv.insert_or_assign(l->varName, v);
        ...
        [&](box<struct Fst>& fst) { 
            ...
            else throw std::runtime_error("Unexpected types for sub-expressions of Fst");
        },
        [&](box<struct Snd>& snd) { 
            ...
            else throw std::runtime_error("Unexpected types for sub-expressions of Snd");
        },
        [&](box<struct Call>& call) {
            ...
            Fun f = c.f;
            std::map<string, Expr> newEnv = makeNewEnvFrom(c.env);
            
            if(!(f.funName == "" || f.funName.empty()))
                newEnv.insert_or_assign(f.funName, e1);
            newEnv.insert_or_assign(f.argName, e2);

            return eval_under_env(f.body, newEnv);
        },
      }, e);
}
...
Expr ToMupList(List<Expr> list) {
    if(list.isEmpty())
        return AUnit();
    else {
        Expr head = list.head();
        Expr tail = ToMupList(list.tail());
        return APair(head, tail);
    }
}

List<Expr> FromMupList(Expr list) {
    if(is<AUnit>(list))
        return List<Expr>();
    else {
        APair ap = *std::get<box<struct APair>>(list);
        Expr head = ap.e1;
        Expr tail = ap.e2;
        return List<Expr>(head, FromMupList(tail));
    }
}
...
int main() {
    ...
    // test for ToMupList and FromMupList
    forEach(FromMupList(makeIntList(0, 5)), [](Expr e) {
        std::cout << toString(e) << " ";
    });
    std::cout << std::endl;
    std::cout << toString(ToMupList(makeList(Expr(Int(0)), Expr(Int(1)), Expr(Int(2)), Expr(Int(3)), Expr(Int(4))))) << std::endl;
